...
locationSchema.pre('save', async function (next) {
    if (this.isNew) {
        try {
            ...
            const parentWarehouse = await mongoose.model('Warehouse').findById(this.warehouse);
            if (!parentWarehouse) {
            ...
            if (lastLocation && lastLocation.barcode_key) {
                ...
                const lastSequence = parseInt(lastLocation.barcode_key.replace(warehouseCode, ''), 10);
