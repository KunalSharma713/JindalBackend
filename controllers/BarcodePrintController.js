...
exports.getLocationBarcodes = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=location_barcodes_page${page}.pdf`);
    ...
    for (const location of locations) {
      try {
        ...
        doc.roundedRect(xPos, yPos, barcodeWidth, barcodeHeight, 15)
           .stroke();
        ...
        console.error('Barcode generation error for location:', location, barcodeError);
        continue;
      }
    }
...
exports.getPalletBarcodes = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const totalItems = await PalletBarcode.countDocuments();
    const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);

    const pallets = await PalletBarcode.find()
      .skip((page - 1) * ITEMS_PER_PAGE)
      .limit(ITEMS_PER_PAGE);

    // Debug log to see pallet structure
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=pallet_barcodes_page${page}.pdf`);
    ...
    for (const pallet of pallets) {
      try {
        ...
        const barcodeText = pallet.barcode_key || pallet.barcode || pallet.barcodeKey;
        
        if (!barcodeText) {
          console.error('No barcode found for pallet. Available fields:', Object.keys(pallet._doc));
          continue;
        }
