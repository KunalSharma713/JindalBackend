...
const getLocationBarcodesWeb = async (req, res) => {
  try {
    ...
    const location = await Location.findById(locationId); // Fixed: Avoid overwriting model
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=location_barcodes_${locationId}.pdf` // Fixed: `page` is not defined
    );
    ...
    try {
      ...
      if (!location.barcode_key) {
        ...
        return res
          .status(400)
          .json({ message: "No barcode key found for this location." });
      }
      ...
      if (!barcodeBuffer || barcodeBuffer.length === 0) {
        console.error("Empty barcode buffer for:", location.barcode_key);
        return res.status(400).json({ message: "Barcode generation failed." });
      }
