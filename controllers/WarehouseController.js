...
const getAllWarehouses = async (req, res) => {
    try {
        ...
        const warehouses = await Warehouse.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit);
...
const deleteWarehouse = async (req, res) => {
    try {
        ...
        if (locationCount > 0) {
            return res.status(400).json({ message: 'Cannot delete warehouse with associated locations. Please reassign or delete them first.' });
        }
