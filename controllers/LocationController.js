...
const createLocation = async (req, res) => {
  ...
  try {
    ...
    if (!parentWarehouse) {
      return res.status(404).json({ message: "Parent plant not found." });
    }
  ...
  try {
    if (name) {
      ...
      if (!warehouse) {
        return res
          .status(404)
          .json({ message: "No warehouse found with that code." });
      }
      ...
      return res.status(400).json({
        message:
          "Please provide a search query (name, barcode, or warehouseCode).",
      });
    }
    ...
    res.status(500).json({
      message: "Server error while finding location.",
      error: error.message,
    });
  }
};
