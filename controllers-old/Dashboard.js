...
const gettruck = async (req, res, next) => {
  try {
    ...
    const truckinplan = await Shipments.find({
      active: true,
      shipment_status: { $in: ['GateIn', 'Loading', 'Loaded'] },
    ...
    const truckinloading = await Shipments.find({
      active: true,
      shipment_status: { $in: ['Loading',"Loaded","GateOut"] },
      updated_at: { $gte: startOfDay, $lt: endOfDay }
    });

    const shipmentIds = truckinloading.map(s => s._id);
    const statuses = await Shipmentstatus.find({
      shipmentID: { $in: shipmentIds },
      shipment_status: { $in: ['Loading'] },
      updated_at: { $gte: startOfDay, $lt: endOfDay }
    })
    const totaltruckinloading = statuses.length;
...
const getLogisticMetrics = async (req, res, next) => {
  try {
    ...
    const avrage_shipment_per_days = await Shipments.find({
      active: true,
      shipment_status: { $in: ["Planned", 'Assigned', 'Confirmed', "GateIn", "Loading", "Loaded", "GateOut"] },
      created_at: { $gte: thirtyDaysAgo }
    });

    const totalavrage_shipment_per_days = avrage_shipment_per_days.length / 30;
...
const getSupplierComplianceMetrics = async (req, res, next) => {
  try {
    ...
    const filters = {
      active: true,
      updated_at: { $gte: thirtyDaysAgo },
      shipment_status: ["GateIn", "Loading", "Loaded", "GateOut"],
    };
